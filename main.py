...
def _enforce_public_webui_security(host: str) -> None:
    ...
    if is_auth_enabled():
        return
    if os.getenv("ALLOW_INSECURE_PUBLIC_BIND", "false").strip().lower() in {
        "1", "true", "yes", "on",
    }:
        logger.critical("SECURITY WARNING: public WebUI bind %s without authentication was explicitly allowed.", host)
        return
    raise RuntimeError(
        f"Refusing insecure public WebUI bind {host}: enable ADMIN_AUTH_ENABLED "
        "or explicitly set ALLOW_INSECURE_PUBLIC_BIND=true for a trusted network."
    )
...
def main() -> int:
    ...
    logger.info("=" * 60)
    logger.info("金股睿析（JinguRuiXi）启动")
    ...
    if start_serve:
        ...
        _enforce_public_webui_security(args.host)
