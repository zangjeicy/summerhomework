...
def create_app(static_dir: Optional[Path] = None) -> FastAPI:
    ...
    # 创建 FastAPI 实例
    app = FastAPI(
        title="JinguRuiXi API",
        description=(
            "金股睿析（JinguRuiXi）API，作者：张峻诚。\n\n"
            "覆盖 A股/港股/美股等市场的自选股智能分析。\n\n"
            "## 功能模块\n"
            "- 股票分析：触发 AI 智能分析\n"
            "- 历史记录：查询历史分析报告\n"
            "- 股票数据：获取行情数据\n\n"
...
<title>金股睿析 - Frontend Not Built</title>
<style>
...
<p class="status">JinguRuiXi API 1.0.0 &bull; 作者 张峻诚 &bull; <a href="/api/health">/api/health</a></p>
</div></body></html>"""
    ...
    @app.get("/api/ready", tags=["Health"], summary="就绪检查")
    async def readiness_check() -> JSONResponse:
        checks = {"database": False, "migrations": False, "frontend": (static_dir / "index.html").is_file(), "services": hasattr(app.state, "system_config_service")}
        error = ""
        try:
            ...
            checks["migrations"] = bool(getattr(database, "_initialized", False))
        except Exception as exc:
            error = str(exc)
        ready = all(checks.values())
        payload = {"status": "ready" if ready else "not_ready", "timestamp": datetime.now().isoformat(), "checks": checks}
        if error:
            payload["error"] = error
