...
> **如意二开层**：本仓库是开源项目 daily_stock_analysis 的二开 fork，对外品牌名「金股睿析 / JinguRuiXi」，作者为张峻诚。`.claude/rules/`（`git.md` 提交规范、`blog.md` 博客规范）是本项目专属约定，与本文件或全局默认冲突时**以 `.claude/rules/` 为准**（例如：本项目提交信息用详细中文并保留 `Co-Authored-By`；博客写作见 `blog.md`）。二开层只约束我们自己产生的改动；从 `upstream` 同步进来的提交与文件保持原貌。
