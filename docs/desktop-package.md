...
桌面端发行配置统一读取仓库根目录 product-release.json。只有在 releaseRepository 明确配置为经确认的公开官方仓库、且 desktopUpdateChannel 启用时才允许检查更新。当前未配置稳定官方 Release 仓库，因此自动更新默认关闭，设置页和日志会明确说明，不会回退查询上游 Release。产品 SemVer、课程版本和 v0.1.0 教学 ZIP 是三套独立语义。