...
> 金股睿析当前未配置经确认的稳定 Release 仓库；上游 Release 仅代表上游项目，不作为本产品下载或更新渠道。

## [Unreleased]

- [修复] 桌面端发行与支持渠道改为读取集中产品元数据；在未配置经确认的金股睿析 Release 仓库时禁用自动更新并明确提示，不再查询或安装上游版本。
- [修复] 新增 liveness/readiness 分层，readiness 检查 SQLite、迁移初始化、Web 静态入口和必要服务；Docker 健康检查失败时返回非零，Compose 默认仅映射到 127.0.0.1。
- [修复] WebUI 非回环监听且管理员认证关闭时默认拒绝启动；仅允许通过 ALLOW_INSECURE_PUBLIC_BIND=true 显式豁免并输出高危警告。
- [测试] CI 同时保护 main/dev，并为 Web、Desktop 与 Docker 增加单测、核心 Playwright、Windows unpacked 构建及真实容器 readiness 门禁。
- [改进] Web 默认主题升级为“如意国风科技蓝”，以 GPT 生成的月白青瓷 / 深海云境双主题山海主视觉替换通用赛博背景，补齐系统主题跟随、可读性与 reduced-motion 支持，并将卡片圆角、透明度、背景模糊、边框与阴影收口为统一主题令牌。

- [文档] 新增常用金融数据爬取源选型指南，整理国内外行情、基本面、宏观与资讯来源及生产接入规范
- [新功能] 北科大 2026 示例新增 Tushare 综合应用，合并基础信息、日线、复权、估值和资金流向并输出可复用数据
- [测试] 北科大 2026 示例新增 Tushare Token 真实可用性验证，安全读取本地 `key.txt` 并校验上交所交易日历响应
- [改进] 重整北科大 2026 上市公司示例，以 5529 行 28 列完整数据为统一入口，并增加可校验备份、旧版归档和连续编号查询脚本
- [新功能] 新增硅基流动 API Key 只读可用性检查脚本，默认安全读取根目录 `key.txt` 并验证模型列表接口
- [改进] Web 首页改为视口自适应宽屏布局：顶部与两级侧栏使用固定槽位，报告区占满剩余空间并独立滚动
- [改进] 将 Web、API 与多语言项目文档的对外品牌统一为“金股睿析 / JinguRuiXi”，并补充作者张峻诚信息
