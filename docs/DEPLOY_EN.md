...
## Secure binding and health checks

/health and /api/health are liveness probes. /ready and /api/ready are readiness probes covering SQLite access, database initialization/migrations, the Web static entry point, and required service initialization. Docker HEALTHCHECK uses /api/ready.

Docker Compose publishes to host 127.0.0.1 by default. A non-loopback WebUI bind requires ADMIN_AUTH_ENABLED=true. ALLOW_INSECURE_PUBLIC_BIND=true is an explicit unsafe override for an isolated trusted LAN and emits a prominent warning.