...
/health 与 /api/health 是 liveness，只表示 HTTP 进程存活；/ready 与 /api/ready 是 readiness，会检查 SQLite、数据库初始化/迁移、Web 静态入口和必要服务。Docker HEALTHCHECK 使用 /api/ready。

Docker Compose 默认只将服务映射到宿主机 127.0.0.1。直接以非回环地址启动 WebUI 时，必须启用 ADMIN_AUTH_ENABLED=true；仅在受信隔离内网中才可显式设置 ALLOW_INSECURE_PUBLIC_BIND=true，此豁免会输出醒目的安全警告。