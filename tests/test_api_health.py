...
class HealthEndpointTestCase(unittest.TestCase):
    ...
    def test_readiness_fails_when_frontend_is_missing(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            with TestClient(create_app(static_dir=Path(temp_dir))) as client:
                resp = client.get("/api/ready")
        self.assertEqual(resp.status_code, 503)
        self.assertFalse(resp.json()["checks"]["frontend"])

    def test_readiness_succeeds_with_database_services_and_frontend(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            static_dir = Path(temp_dir)
