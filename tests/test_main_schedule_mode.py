...
class MainScheduleModeTestCase(unittest.TestCase):
    ...
    def test_public_webui_bind_is_rejected_when_auth_is_disabled(self) -> None:
        with patch("src.auth.is_auth_enabled", return_value=False), \
            ...
            os.environ.pop("ALLOW_INSECURE_PUBLIC_BIND", None)
            with self.assertRaises(RuntimeError):
                main._enforce_public_webui_security("0.0.0.0")

    def test_loopback_webui_bind_is_allowed_when_auth_is_disabled(self) -> None:
        with patch("src.auth.is_auth_enabled", return_value=False):
            main._enforce_public_webui_security("127.0.0.1")

    def test_public_webui_bind_is_allowed_when_auth_is_enabled(self) -> None:
        with patch("src.auth.is_auth_enabled", return_value=True):
            main._enforce_public_webui_security("0.0.0.0")

    def test_public_webui_bind_explicit_override_warns(self) -> None:
        with patch("src.auth.is_auth_enabled", return_value=False), \
             patch.dict("os.environ", {"ALLOW_INSECURE_PUBLIC_BIND": "true"}), \
             patch("main.logger.critical") as critical_log:
            main._enforce_public_webui_security("0.0.0.0")
