...
test.skip('configured auto download prompt falls back to error when install path fails', async (t) => {
  const updaterEvents = {};
...
test.skip('configured auto update backup copies AlphaSift hotspot detail directories recursively', async (t) => {
  const updaterEvents = {};
...
test('Ruyi release metadata disables updates when no official repository is configured', (t) => {
  const mainModule = loadMainModule(t);
  assert.equal(mainModule.PRODUCT_RELEASE_METADATA.productName, '金股睿析');
  assert.equal(mainModule.PRODUCT_RELEASE_METADATA.author, '张峻诚');
  assert.equal(mainModule.PRODUCT_RELEASE_METADATA.releaseRepository, '');
  assert.equal(mainModule.DESKTOP_UPDATE_CONFIGURED, false);
  assert.equal(mainModule.LATEST_RELEASE_API_URL, '');
  assert.doesNotMatch(JSON.stringify(mainModule.PRODUCT_RELEASE_METADATA), /ZhuLinsen|daily_stock_analysis/);
});