...
function loadProductReleaseMetadata() {
  const candidates = [
    app.isPackaged && process.resourcesPath ? path.join(process.resourcesPath, 'product-release.json') : null,
    path.join(appRootDev, 'product-release.json'),
  ].filter(Boolean);
  const metadataPath = candidates.find((candidate) => fs.existsSync(candidate));
  if (!metadataPath) {
    throw new Error('product-release.json is required for desktop release safety');
  }
  return JSON.parse(fs.readFileSync(metadataPath, 'utf-8'));
}
const PRODUCT_RELEASE_METADATA = loadProductReleaseMetadata();
const RELEASE_REPOSITORY = String(PRODUCT_RELEASE_METADATA.releaseRepository || '').trim();
const DESKTOP_UPDATE_CHANNEL = String(PRODUCT_RELEASE_METADATA.desktopUpdateChannel || 'disabled').trim();
const DESKTOP_UPDATE_CONFIGURED = DESKTOP_UPDATE_CHANNEL !== 'disabled' && /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(RELEASE_REPOSITORY);
const [GITHUB_OWNER = '', GITHUB_REPO = ''] = DESKTOP_UPDATE_CONFIGURED ? RELEASE_REPOSITORY.split('/') : [];
const RELEASES_PAGE_URL = DESKTOP_UPDATE_CONFIGURED ? 'https://github.com/' + RELEASE_REPOSITORY + '/releases' : '';
const LATEST_RELEASE_API_URL = DESKTOP_UPDATE_CONFIGURED ? 'https://api.github.com/repos/' + RELEASE_REPOSITORY + '/releases/latest' : '';
...
async function performDesktopUpdateCheck({ manual = false, notify = false } = {}) {
  if (!DESKTOP_UPDATE_CONFIGURED) {
    const state = setDesktopUpdateState({
      status: UPDATE_STATUS.IDLE,
      updateMode: UPDATE_MODE.MANUAL,
      currentVersion: resolveDesktopVersion(),
      releaseUrl: '',
      checkedAt: new Date().toISOString(),
      message: '当前没有配置金股睿析官方更新源，自动更新已关闭。',
    });
    logLine('[update] disabled: no official release repository is configured');
    return state;
  }
...
async function createWindow() {
  ...
  const healthUrl = buildBackendUrl(backendConnectHost, port, '/api/ready');
  ...
  try {
    ...
    if (!restoreFailed && DESKTOP_UPDATE_CONFIGURED) {
      ...
      logLine('[update] startup check skipped: official update source is not configured');
    }
  } catch (error) {
