...
describe('ReportOverview', () => {
  it('uses a fluid report column and a fixed-width metrics rail on wide screens', () => {
    render(<ReportOverview meta={baseMeta} summary={baseSummary} />);

    expect(screen.getByTestId('report-overview-layout')).toHaveClass(
      'grid-cols-1',
      '2xl:grid-cols-[minmax(0,1fr)_18rem]',
    );
    expect(screen.getByTestId('report-overview-main')).toHaveClass('min-w-0');
    expect(screen.getByTestId('report-overview-main')).not.toHaveClass('lg:col-span-2');
    expect(screen.getByTestId('report-overview-rail')).toHaveClass('min-w-0');
    expect(screen.getByTestId('report-overview-rail')).not.toHaveClass('fixed', 'sticky');
  });
