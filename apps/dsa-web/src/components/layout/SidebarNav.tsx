...
import { Activity, BarChart3, Bell, Brain, BriefcaseBusiness, Gauge, Home, LogOut, MessageSquareQuote, Search, Settings2 } from 'lucide-react';
...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'ml-dashboard', labelKey: 'layout.nav.mlDashboard', to: '/ml-dashboard', icon: Brain },
  ...
  return (
    <div className="flex h-full flex-col">
      <div
        className={cn(
          'flex items-center',
          isRail ? 'mb-4 flex-col justify-center gap-1.5 pt-1 text-center' : 'mb-4 gap-2 px-1',
      ...
      >
        <BrandMark
          variant="icon"
          className={cn(
            'shrink-0 shadow-[0_12px_28px_var(--nav-brand-shadow)]',
            isRail ? 'h-9 w-9 rounded-[1rem]' : 'h-10 w-10 rounded-2xl'
          )}
        />
        {!collapsed ? (
          <div className={cn('min-w-0', isRail ? 'w-full' : '')}>
            <p className={cn('truncate font-semibold text-foreground', isRail ? 'text-[0.95rem] leading-none' : 'text-sm')}>
              {APP_BRAND.chineseName}
            </p>
            <p className={cn(
              'mt-0.5 text-secondary-text',
              isRail ? 'break-all text-[8px] leading-[1.15]' : 'truncate text-[10px] leading-tight'
            )}>
                {APP_BRAND.englishName} · {APP_BRAND.author}
            </p>
          </div>
        ) : null}
      </div>
