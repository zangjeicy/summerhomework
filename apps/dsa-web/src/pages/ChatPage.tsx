...
const ChatPage: React.FC = () => {
  ...
  // Set page title
  useEffect(() => {
    document.title = `问股 - ${APP_BRAND.chineseName}`;
  }, []);
  ...
  return (
    ...
    >
      {/* Desktop sidebar */}
      <div className="surface-card hidden h-full w-64 flex-shrink-0 flex-col md:flex">
        {sidebarContent}
      </div>
      ...
      {sidebarOpen && (
        ...
        >
          ...
          <div
            className="surface-floating absolute bottom-0 left-0 top-0 flex w-72 flex-col overflow-hidden border-r border-white/10"
      ...
      <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <div className="surface-card relative z-10 flex min-h-0 flex-1 flex-col border-white/6">
          {/* Messages */}
          <ScrollArea
            ...
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  ...
                  className="max-w-2xl border-dashed border-border/60"
                  icon={(
                    <svg
                      className="h-8 w-8"
                      fill="none"
