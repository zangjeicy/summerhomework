...
describe('HomePage', () => {
  ...
  it('renders the dashboard workspace and auto-loads the first report', async () => {
    ...
    expect(dashboard).toHaveClass('h-full', 'min-h-0', 'overflow-hidden');
    expect(dashboard).not.toHaveClass('h-[calc(100vh-5rem)]', 'lg:h-[calc(100vh-2rem)]');
    expect(screen.getByTestId('home-workspace')).toHaveClass('h-full', 'w-full', 'min-h-0', 'min-w-0');
    expect(screen.getByTestId('home-workspace')).not.toHaveClass('lg:max-w-6xl', 'mx-auto');
    expect(dashboard.querySelector('.flex-1.flex.min-h-0.overflow-hidden')).toBeTruthy();
    expect(screen.getByTestId('home-dashboard-header')).toHaveClass('shrink-0', 'lg:h-16');
    expect(screen.getByTestId('home-dashboard-sidebar')).toHaveClass('shrink-0', 'lg:w-72');
    expect(screen.getByTestId('home-dashboard-scroll')).toHaveClass('flex-1', 'min-h-0', 'overflow-y-auto', 'overflow-x-hidden');
    expect(await screen.findByTestId('home-report-content')).toHaveClass('w-full', 'max-w-none');
