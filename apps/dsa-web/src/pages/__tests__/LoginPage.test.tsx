...
describe('LoginPage', () => {
  ...
  it('renders the project lockup on the login page', () => {
    useAuthMock.mockReturnValue({
      login: vi.fn(),
      passwordSet: true,
      setupState: 'enabled',
    ...
    expect(screen.getByRole('img', { name: '如意金股 RuyiDailyStockAnalysis' })).toBeInTheDocument();
    expect(container.firstElementChild).toHaveAttribute('data-brand-theme', 'ruyi-tech-blue');
    expect(screen.getByTestId('ruyi-login-backdrop')).toHaveAttribute('aria-hidden', 'true');
