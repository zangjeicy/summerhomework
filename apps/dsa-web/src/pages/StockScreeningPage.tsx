  ...
  return (
    <AppPage className="max-w-6xl space-y-6 pb-12 pt-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-secondary-text">开启后通过内置 AlphaSift 适配层生成候选股票，并补充如意金股数据与新闻</p>
          </div>
        </div>
      ...
      <section className="surface-card p-4">
        ...
        {!hotspotsExpanded ? (
          <div className="surface-inset flex flex-col gap-2 px-4 py-3 text-sm text-secondary-text sm:flex-row sm:items-center sm:justify-between">
            <span>
              {hotspots.length > 0
          ...
          <div className="surface-inset border-dashed px-4 py-6 text-sm text-secondary-text">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {hotspots.map((item, index) => {
              ...
              <button
                ...
                className={`surface-inset group relative min-h-[116px] px-3 py-3 text-left transition-all ${
                  selected
                    ...
                    : 'border-border/80 hover:-translate-y-0.5 hover:border-orange-300/70 hover:shadow-soft-card'
                }`}
                type="button"
                ...
                <div>
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    {(hotspotDetail.stocks || []).slice(0, 10).map((stock) => (
                      <div key={`${stock.code || stock.name}`} className="surface-inset p-3">
      ...
      <section className="surface-card border-cyan/35 p-4">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold text-foreground">选择策略</h2>
            <p className="mt-1 text-xs text-secondary-text">策略来自 AlphaSift；如意金股会对候选补充行情、基本面和新闻上下文。</p>
          </div>
        ...
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
          {loadingStrategies ? (
            <div className="surface-inset border-dashed p-4 text-sm text-secondary-text">
              正在读取可用策略...
            </div>
          ) : strategies.length === 0 ? (
            <div className="surface-inset border-dashed p-4 text-sm text-secondary-text">
            ...
            strategies.map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  className={`surface-inset min-h-28 p-4 text-left transition-all ${
                    selected
                      ? 'border-cyan bg-cyan/10 shadow-[0_0_0_1px_hsl(var(--primary)/0.15),0_16px_36px_hsl(var(--primary)/0.12)]'
                      : 'border-border/80 hover:border-cyan/45 hover:bg-hover/70'
                  }`}
                  type="button"
                  disabled={loading}
      ...
      <section className="surface-card p-4">
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-secondary-text">
              AlphaSift 返回候选后，如意金股会对前几名补充行情、基本面、新闻和辅助摘要。
            </p>
          </div>
        ...
        {candidates.length === 0 ? (
          <div className="surface-inset border-dashed px-5 py-10 text-center">
                            ...
                            <div className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
                              <div className="space-y-3">
                                ...
                                {item.dsaAnalysisSummary ? (
                                  <div>
                                    <p className="text-xs font-semibold text-secondary-text">如意金股增强摘要</p>
                              ...
                              <div className="space-y-3">
                                ...
                                <div>
                                  <p className="text-xs font-semibold text-secondary-text">如意金股新闻</p>
                                  {dsaNews.length > 0 ? (
                                ...
                                {dsaWarnings.length > 0 ? (
                                  <div>
                                    <p className="text-xs font-semibold text-secondary-text">如意金股增强提示</p>
